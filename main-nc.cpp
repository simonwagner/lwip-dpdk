...
void ui_thread() {
    ...
    while(true) {
        bytes input(4*1024);
        size_t read_input = fread(input.data(), 1, 4*1024, stdin);
        if(read_input < 4*1024) {
            quit = true;
        }
        input.resize(read_input);
        /*while(kbhit()) {
...

        {
            ...
            if(input_queue_size > 600) {
                this_thread::sleep_for(std::chrono::milliseconds(10)); //limit sending speed
            }
        }
...
dispatch_ui_input()
{
    ...
    while(!input_cursor.empty()) {
        ...
        if(available == 0) {
            break; //back off and try again later
        }
